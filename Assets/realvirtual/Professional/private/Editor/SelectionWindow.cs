...
namespace realvirtual
{
    ...
    public class SelectionWindow : EditorWindow
    {
        ...
        private static Hashtable hashGroupIndexGroup;
        private static Hashtable hashGroupIndexPrefix;
        ...
        private static void InitSettings()
        {
            settings = UnityEditor.AssetDatabase.LoadAssetAtPath(
                "Assets/realvirtual/Professional/private/Editor/SelectionWindowSettings.asset",
        ...
        public void Awake()
        {
            ...
            {
                foreach (var group in Global.realvirtualcontroller.HiddenGroups)
                {
        ...
        static void GetGroups()
        {
            ...
            hashGroupIndexGroup = new Hashtable();
            hashGroupIndexPrefix = new Hashtable();
            bool PrefixAvailable = false;
            string groupIndex;
            foreach (Group group in groups)
            {
                ...
                {
                    groupIndex = group.GroupNamePrefix.name + "_" + group.GroupName;
                ...
                if (!Groups.Contains(groupIndex))
                {
                    ...
                    hashGroupIndexGroup.Add(groupIndex,group.GroupName);
                    if(PrefixAvailable)
                        hashGroupIndexPrefix.Add(groupIndex,group.GroupNamePrefix);
                }
            }
        ...
        public List<GameObject> GetAllWithGroup(string group)
        {
            ...
            {
                var gr = (Group) comp;
                GameObject prefix = null;
                if (EditorUtility.IsPersistent(gr.transform.root.gameObject))
                    continue;
                if (gr.GroupName == (string)hashGroupIndexGroup[group])
                {
                    if (hashGroupIndexPrefix.Contains(group))
                    {
                        prefix = (GameObject)hashGroupIndexPrefix[group];
                        if(gr.GroupNamePrefix==prefix)
        ...
        private void SelectVisible(bool visible)
        {
            ...
            foreach (var comp in groupcomps)
            {
                ...
                if (EditorUtility.IsPersistent(gr.transform.root.gameObject))
                    continue;
                if (gr.transform.IsChildOf(Global.realvirtualcontroller.transform))
                    continue;
        ...
        private void ToGroup(string groupIndex, bool includingsub)
        {
            bool hidden = false;
            string group;
            GameObject groupPrefix = null;
            if(hashGroupIndexGroup.Contains(groupIndex))
            {
                group = (string)hashGroupIndexGroup[groupIndex];
                if (hashGroupIndexPrefix.Contains(groupIndex))
                {
                    groupPrefix = (GameObject)hashGroupIndexPrefix[groupIndex];
                }
            }
            else
            {
                group = groupIndex;
                hashGroupIndexGroup.Add(groupIndex,group);
            }

            if (Global.g4acontrollernotnull)
                if (Global.realvirtualcontroller.GroupIsHidden(group))
                    hidden = true;
            ...
            foreach (var obj in objs)
            {
                ...
                if (!existing)
                {
                    ...
                    groupcom.GroupNamePrefix = groupPrefix;
                    if (hidden)
                    ...
                    foreach (var obje in objs)
                    {
                        ...
                        var go = (GameObject) obje;
                            ...
                            if (Global.g4acontrollernotnull && considerhidden)
                            {
                                ...
                                if (group != null)
                                    ishidden = Global.realvirtualcontroller.GroupIsHidden(group.GroupName);
                            }
        ...
        void OnGUI()
        {
            ...
            foreach (var group in Groups)
            {
                ...
                if (Global.g4acontrollernotnull)
                {
                    if (Global.realvirtualcontroller.HiddenGroups.Contains(group))
                    {
                        grouphidden = true;
                    }
                }
                ...
                if (GUILayout.Button("-", GUILayout.Width(width / 18)))
                {
                    RemoveThisGroup((string)hashGroupIndexGroup[group], false);
                }
                ...
                if (GUILayout.Button("--", GUILayout.Width(width / 18)))
                {
                    RemoveThisGroup((string)hashGroupIndexGroup[group], true);
                }
                ...
                if (GUILayout.Button(text, hiddenstyle, GUILayout.Width(width / 8)))
                {
                    ...
                    {
                        if (!grouphidden)
                            Global.realvirtualcontroller.AddHideGroup(group);
                        else
                            Global.realvirtualcontroller.RemoveHideGroup(group);
                    }
                ...
                if (isolategroup == (string)hashGroupIndexGroup[group])
