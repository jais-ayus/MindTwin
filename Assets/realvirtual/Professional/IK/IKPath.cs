...
#pragma warning disable 0414
    ...
    //! This professional component manages robot trajectories by sequencing multiple IK targets with configurable
    //! interpolation methods (PTP, linear), speed control, and signal integration. Essential for programming
    //! pick-and-place operations, welding paths, assembly sequences, and material handling in virtual commissioning.
    public class IKPath : BehaviorInterface, IXRPlaceable
    {
        ...
        [ReadOnly] public bool PathIsFinished = false; //!< Is the path finished?
        [ReadOnly] public bool LinearPathActive = false; //!< Is the linear path active?
        [ReadOnly] public float LinearPathPos = 0; //!< Position of the linear path
        [ReadOnly] public IKTarget CurrentTarget; //!<  Current target of the path
        [ReadOnly] public IKTarget LastTarget; //!< Last target of the path
        [ReadOnly] public int NumTarget; //!< Number of the current target
        [ReadOnly] public bool WaitForSignal; //!< Is the path waiting for a signal?
        [ReadOnly] public RobotIK RobotIK; //!<  RobotIK component - assigned robot to this path
        [ReadOnly] public Vector3 LastPlannedPosition; //!< Last planned position of the robot
        [ReadOnly] public Quaternion LastPlannedRotation;
        ...
        void StartDriveLinear(IKTarget target)
        {
            ...
            if (LastPlannedRotation == Quaternion.identity)
                LinearPathStartRot = RobotIK.GetTCPRotGlobal();
            else
                LinearPathStartRot = LastPlannedRotation;
        ...
        private void DriveOnOnAtPosition(Drive drive)
        {
            ...
            if (drivesatposition.Count == 6)
            {
                ...
                LastPlannedRotation = CurrentTarget.transform.rotation;
                AtTarget();
            }
        }
        ...
        void Start()
        {
            ...
            LastPlannedRotation = Quaternion.identity;
