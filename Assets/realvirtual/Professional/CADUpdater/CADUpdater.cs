...
{
    //! Intelligently synchronizes CAD model updates while preserving Unity-specific modifications and component assignments.
    //! This professional tool automates the complex process of updating imported CAD assemblies by comparing metadata,
    //! detecting changes in geometry and structure, and selectively updating meshes and positions. Essential for
    //! maintaining virtual commissioning models synchronized with engineering CAD data while preserving simulation logic,
    //! scripts, and manual adjustments. Supports various CAD formats with configurable update strategies.
    public class CADUpdater : MonoBehaviour
    {
        ...
        public bool ChainIDs=true; //!< Chain IDs of all upper Parts to create an ID (only needed if no correct ID metadata is available)
        public bool ForceUniqueIDs=true;

        public bool UseJTVersionNumber; //!< Use JT Version Number (if correct JT Version number is exported)
        public bool CompareOnlyMeshLength = false; //!< Compare only the Lenght of the Mesh arrays not the content itself
        public bool UpdatePositions=true; //!< Update Positions if Position is moved
        public bool UpdateMeshes=true; //!< Update Meshes if Mesh is changed
        public bool DeleteParts=true; //!< Delete Parts which are not existing any more in the imported new CAD data
        public bool AddNewParts=true; //!< Add new Parts
        public List<CAD> ToDeleteWithScripts; //!< List with Objects which would be deleted and which are having realvirtual scripts on it
        ...
        {
            // Remove all CADData from Current and Update if not null
            if(CADCurrent != null)
            {
                RemoveCADComponent(CADCurrent);
            }

            if(CADUpdate != null)
            {
                RemoveCADComponent(CADUpdate);
            }
        }

        private void RemoveCADComponent(GameObject parent)
        {
            foreach(Transform child in parent.transform)
            {
                CAD cadComponent = child.gameObject.GetComponent<CAD>();
                if(cadComponent != null)
                {
                    DestroyImmediate(cadComponent);
                }

                if(child.childCount > 0)
                {
                    RemoveCADComponent(child.gameObject);
                }
            }
        }
        ...
        public void CheckStatus()
        {
            ...
            #if UNITY_EDITOR
            // If in UnityEdctiro check if CADCurrent and CADUpdate are not null- if they are null write a prompt message
            if (CADCurrent == null || CADUpdate == null)
            {
                EditorUtility.DisplayDialog("Error", "CADCurrent or CADUpdate is not set", "OK");
                return;
            }
            #endif
